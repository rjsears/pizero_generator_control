...
class RebootScheduler:
    ...
    def _execute_reboot(self) -> None:
        ...
        Uses nsenter to run reboot in the host namespace since we're
        running inside a Docker container. Requires privileged: true
        and pid: host in docker-compose.yaml.
        """
        ...
        # Execute reboot in host namespace using nsenter
        ...
        try:
            ...
            # Only mark as rebooted if command succeeded
            self._last_reboot_date = datetime.now().strftime("%Y-%m-%d")
            logger.info("Host reboot command executed successfully")
