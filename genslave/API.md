...
- [Notifications](#notifications)
  ...
  - [GET /api/system/notifications/settings - Get Cooldown](#get-apisystemnotificationssettings---get-cooldown-settings)
  - [POST /api/system/notifications/settings - Update Cooldown](#post-apisystemnotificationssettings---update-cooldown-settings)
  - [POST /api/system/notifications/clear-cooldown - Clear Cooldown](#post-apisystemnotificationsclear-cooldown---clear-cooldown)
- [Scheduled Reboots](#scheduled-reboots)
  - [GET /api/system/reboot-schedule - Get Schedule](#get-apisystemreboot-schedule---get-schedule)
  - [POST /api/system/reboot-schedule - Set Schedule](#post-apisystemreboot-schedule---set-schedule)
  - [POST /api/system/reboot-schedule/enable - Enable/Disable](#post-apisystemreboot-scheduleenable---enabledisable)
- [WiFi Configuration](#wifi-configuration)
  - [GET /api/system/wifi/networks - Scan Networks](#get-apisystemwifinetworks---scan-networks)
  - [POST /api/system/wifi/connect - Connect to Network](#post-apisystemwificonnect---connect-to-network)
  - [GET /api/system/wifi/saved - List Saved Networks](#get-apisystemwifisaved---list-saved-networks)
  - [POST /api/system/wifi/add - Add Known Network](#post-apisystemwifiadd---add-known-network)
  - [POST /api/system/wifi/delete - Delete Saved Network](#post-apisystemwifidelete---delete-saved-network)
- [System Power Control](#system-power-control)
  - [POST /api/system/shutdown - Shutdown System](#post-apisystemshutdown---shutdown-system)
  - [POST /api/system/reboot - Reboot System](#post-apisystemreboot---reboot-system)
...
### GET `/api/system/notifications/settings` - Get Cooldown Settings

Get notification cooldown configuration to prevent notification flapping.
...
```json
{
  ...
  "last_failsafe_notification_at": 1705612800,
  "last_restored_notification_at": 1705612900
}
```

---

### POST `/api/system/notifications/settings` - Update Cooldown Settings

Update notification cooldown periods.

**Request Body:**
```json
{
  "failsafe_cooldown_minutes": 10,
  "restored_cooldown_minutes": 10
}
```

All fields are optional. Values must be between 1-60 minutes.

**Response:**
```json
{
  "success": true,
  "message": "Cooldown settings updated: failsafe=10min, restored=10min"
}
```

---

### POST `/api/system/notifications/clear-cooldown` - Clear Cooldown

Clear notification cooldown to allow immediate notifications.
...
|-------|------|-------------|
| `event_type` | string | `"failsafe"`, `"restored"`, or `null` for both |

**Response:**
```json
{
  "success": true,
  "message": "Cooldown cleared for failsafe notifications"
}
```
...
GenSlave supports scheduled maintenance reboots for system health.

### GET `/api/system/reboot-schedule` - Get Schedule
...
```json
{
  ...
  "minute": 0,
  "running": true,
  "last_reboot_date": "2026-04-20",
  "next_reboot": "2026-04-27T04:00:00"
}
```

| Field | Description |
|-------|-------------|
| `enabled` | Whether scheduled reboots are active |
| `day` | Day of week (`monday`-`sunday`) or `daily` |
| `hour` | Hour (0-23) |
| `minute` | Minute (0-59) |
| `running` | Whether the scheduler is running |
| `last_reboot_date` | Date of last scheduled reboot |
| `next_reboot` | ISO timestamp of next scheduled reboot |

---

### POST `/api/system/reboot-schedule` - Set Schedule
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `enabled` | boolean | Yes | Enable/disable scheduled reboots |
| `day` | string | Yes | `monday`-`sunday` or `daily` |
...
{
  "success": true,
  "message": "Reboot schedule updated: sunday at 04:00"
}
```

> **Note:** Reboots only occur if the generator relay is OFF. If the generator is running at the scheduled time, the reboot is skipped.

---

### POST `/api/system/reboot-schedule/enable` - Enable/Disable

Toggle scheduled reboots without changing the schedule.
...
{
  "success": true,
  "message": "Scheduled reboots disabled"
}
```
...
Manage WiFi networks on the GenSlave Pi.

### GET `/api/system/wifi/networks` - Scan Networks
...
### POST `/api/system/wifi/connect` - Connect to Network
...
{
  "success": true,
  "message": "Successfully connected to MyNetwork",
  "error": null
}
```
...
### GET `/api/system/wifi/saved` - List Saved Networks
...
### POST `/api/system/wifi/add` - Add Known Network

Add a WiFi network for auto-connect (useful for pre-configuring networks).
...
{
  "success": true,
  "message": "WiFi network 'NewNetwork' added successfully. It will auto-connect when available.",
  "error": null
}
```
...
### POST `/api/system/wifi/delete` - Delete Saved Network
...
{
  "success": true,
  "message": "WiFi network 'OldNetwork' deleted successfully.",
  "error": null
}
```
...
Initiate system shutdown. The relay is turned OFF for safety.
...
> **Warning:** The system will be unreachable until manually powered on.

---

### POST `/api/system/reboot` - Reboot System

Initiate system reboot. The relay is turned OFF for safety.
...
> **Note:** GenSlave will automatically restart and become available within 60-90 seconds.
