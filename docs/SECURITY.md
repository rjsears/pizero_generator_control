...
> [!WARNING]
> **Safety scope.** This is software for controlling a generator's start
> contact. It is **not** a safety system. Generator manufacturer safeties
> (overspeed, overtemp, low-oil, CO sensors), Automatic Transfer Switch
> logic, fuel safety, overcurrent protection, and electrical-code compliance
> are out of scope for everything in this document. See the warning at the
> top of the project README.
...
- GenSlave's REST API requires `Authorization: Bearer <API_SECRET>` on every
  state-changing endpoint.
- The secret is auto-generated by `setup.sh` (32 bytes via `openssl rand`).
- Both ends must hold the same secret. The Settings UI provides an atomic
  rotation flow that updates both sides at once.
- Heartbeat traffic also runs over this same authenticated channel and over
...
- [`docs/CLOUDFLARE_TUNNEL_REAL_IP_FIX.md`](CLOUDFLARE_TUNNEL_REAL_IP_FIX.md) — tunnel real-IP details
