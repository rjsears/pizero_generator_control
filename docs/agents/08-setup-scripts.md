...
# GenSlave Communication (port 8001)
...
**NOTE**: GenSlave runs on port **8001** (GenMaster uses 8000).
...
# API health
echo "API Status:"
curl -s http://localhost:8001/api/health | jq . 2>/dev/null || echo "API not responding"
...
# Relay status
echo "Relay Status:"
curl -s http://localhost:8001/api/relay/status | jq . 2>/dev/null
