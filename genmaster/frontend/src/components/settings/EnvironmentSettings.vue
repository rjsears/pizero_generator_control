...
function formatContainerName(name) {
  // Remove genmaster_ prefix and format nicely
  return name
    .replace(/^genmaster_/, '')
    .replace(/_/g, ' ')
    .split(' ')
...
async function closeProgressModal() {
  ...
  if (wasSuccess && backupId) {
    try {
      ...
      a.href = url
      const filename = backup?.filename || `genmaster_full_backup_${new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19)}.tar.gz`
      a.download = filename
...
function downloadRecoveryInstructions() {
  const instructions = `================================================================================
GenMaster - Environment File Recovery Instructions
================================================================================

If you have made changes to environment variables that have caused the system
to become unusable and you cannot access GenMaster, follow these
...
1. SSH to the Docker host that runs your GenMaster system:

   ssh user@your-docker-host

2. Navigate to the GenMaster installation directory:

   cd /path/to/genmaster
   (or wherever you installed GenMaster)
...
10. Access the GenMaster web interface to verify everything works:
...
2. Check individual container:
   docker logs genmaster --tail 100
        ...
        <div class="px-6 py-5 bg-white dark:bg-gray-800">
          ...
          <div class="mt-4 p-4 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800">
            ...
            <p class="text-red-600 dark:text-red-300 text-center text-sm">
              These settings control the core functionality of GenMaster, its supporting containers, and local and remote access. Incorrect values here can lead to partial or complete system failure.
            </p>
          </div>
        ...
        <div class="mx-6 mb-4 p-4 bg-amber-50 dark:bg-amber-500/10 rounded-lg border border-amber-200 dark:border-amber-500/20">
          <div class="text-center">
            ...
            <p class="text-amber-700 dark:text-amber-400 text-xs mb-3">
              If changes cause the GenMaster web interface to become inaccessible, you can recover via SSH. Download these instructions now so you have them if needed.
            </p>
            <button
            ...
            <div class="p-6 overflow-y-auto max-h-[60vh]">
                ...
                <div class="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p class="text-red-600/80 dark:text-red-400/70 text-sm mt-1">
                      If the GenMaster web interface becomes inaccessible, you won't be able to view this page.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div class="space-y-4">
                ...
                <h4 class="font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                  ...
                  Navigate to the GenMaster installation directory
                </h4>
                <div class="ml-8 p-3 bg-gray-100 dark:bg-gray-900 rounded-lg font-mono text-sm text-gray-800 dark:text-gray-200">
