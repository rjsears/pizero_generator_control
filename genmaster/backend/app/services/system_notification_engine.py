...
class SystemNotificationEngine:
    ...
    async def _get_targets_for_level(
        ...
        # Query targets for this event and level. Eagerly load the channel
        # relationship and BOTH the group relationship and its nested channels —
        # the dispatcher iterates `target.group.channels` below, and a missing
        # nested load triggers a lazy load that fails in async sessions and
        # results in zero notifications being sent.
        result = await db.execute(
            ...
            .options(
                ...
                .selectinload(NotificationGroup.channels),
            )
            .where(
