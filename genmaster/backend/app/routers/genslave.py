...
async def get_genslave_reboot_schedule():
    ...
    client = await create_slave_client()
    try:
        response = await client.get_reboot_schedule()

        if not response.success:
            raise HTTPException(
                status_code=500,
                detail=response.error or "Failed to get reboot schedule",
            )

        return RebootScheduleStatus(**response.data)
...
async def set_genslave_reboot_schedule(request: RebootScheduleConfig):
    ...
    client = await create_slave_client()
    try:
        response = await client.set_reboot_schedule(
        ...
        if not response.success:
            raise HTTPException(
                status_code=500,
                detail=response.error or "Failed to set reboot schedule",
            )

        return RebootScheduleResponse(**response.data)
...
async def set_genslave_reboot_schedule_enabled(request: RebootScheduleEnableRequest):
    ...
    client = await create_slave_client()
    try:
        response = await client.set_reboot_schedule_enabled(request.enabled)

        if not response.success:
            raise HTTPException(
                status_code=500,
                detail=response.error or "Failed to set reboot schedule enabled",
            )
