...
generate_env_file() {
    ...
    if [ -f "${SCRIPT_DIR}/.env" ]; then
        ...
        local EXISTING_GENSLAVE_API_URL EXISTING_GENSLAVE_API_SECRET
...
EOF
  ...
  certbot:
    ...
    entrypoint: /bin/sh -c "trap exit TERM; while :; do certbot renew --no-random-sleep-on-renew \${DNS_CERTBOT_FLAGS:-} --deploy-hook 'docker exec genmaster_nginx nginx -s reload' || true; sleep 12h & wait $${!}; done;"
    networks:
      - genmaster-internal
      - genmaster-external
EOF
...
obtain_ssl_certificate() {
    ...
    if ! $DOCKER_SUDO docker run --rm \
        ...
        alpine \
        sh -c "cp -a /source/* /etc/letsencrypt/ && ls -la /etc/letsencrypt/live/${DOMAIN}/"; then
